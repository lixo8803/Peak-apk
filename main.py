...
# Versión completa para APK Android
...
class LicenseManager:
    ...
    def check_activation(self):
            ...
            if expiry == "9999-12-31":
                ...
                try:
                    ...
                    if expiry_date > datetime.now():
                        return True, f"Válida hasta {expiry}"
                    else:
                        self.validated = False
                        self.license_data = None
                except:
        ...
        if os.path.exists(self.licencia_file):
            try:
                ...
                self.is_trial = False
                return True, f"Licencia válida - {licencia.get('duracion', '')}"
            except:
                pass
    ...
    def cargar_licencia_archivo(self, ruta_archivo):
        try:
            with open(ruta_archivo, 'r') as f:
                data = json.load(f)
            with open(self.licencia_file, 'w') as f:
                json.dump(data, f, indent=2)
            return self.check_activation()
        except Exception as e:
    ...
    def remove_account(self, account_id):
        for i, acc in enumerate(self.accounts):
            if acc.account_id == account_id:
                ...
                for new_id, remaining_acc in enumerate(self.accounts, 1):
                    remaining_acc.account_id = new_id
                return True, f"Cuenta {removed.username} eliminada"
...
class PeakScreen(Screen):
    ...
    def build_tab_principal(self):
        ...
        # Pérdidas
        self.loss_label = Label(text="Pérdidas: 0/3", font_size=13,
                                color=get_color_from_hex(COLORES['blanco']),
                                size_hint_y=None, height=25)
        ...
        color_box.add_widget(Label(text="Últimos:",
                                   color=get_color_from_hex(COLORES['blanco']),
        ...
        pred_box.add_widget(Label(text="Predicción:",
                                  color=get_color_from_hex(COLORES['blanco']),
    ...
    def build_tab_cuentas(self):
        ...
        self.pass_input = TextInput(hint_text="Contraseña", font_size=11,
                                    background_color=get_color_from_hex(COLORES['card']),
                                    foreground_color=get_color_from_hex(COLORES['blanco']),
    ...
    def add_account(self, instance):
        ...
        if not username or not password:
            self.log("ERROR: Ingresa usuario y contraseña")
            return
